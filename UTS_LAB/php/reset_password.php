...
if (!isset($_SESSION['reset_username'])) {
    echo "<script>alert('Access denied!'); window.location.href='forgot_password.php';</script>";
    exit();
}
    ...
    <div class="max-w-md mx-auto mt-10 bg-white shadow-md rounded px-8 pt-6 pb-8">
        ...
        <form method="POST" action="reset_password.php">
            <div class="mb-4 relative">
                ...
                <span class="toggle-password pt-6" onclick="togglePassword('new_password', this)">👁️</span>
            </div>
            <div class="mb-4 relative">
                <label class="block text-gray-700 text-sm font-bold mb-2" for="confirm_password">Confirm Password</label>
                <input type="password" name="confirm_password" id="confirm_password" required
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
                <span class="toggle-password pt-6" onclick="togglePassword('confirm_password', this)">👁️</span>
            </div>
            <div class="flex items-center justify-between">
                ...
                <a href="login.php" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Cancel</a>
            </div>
        </form>
    </div>
